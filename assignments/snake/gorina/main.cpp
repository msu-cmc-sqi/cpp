...
class Game {
    ...
    void printLevel() {
        int max_x = 0, max_y = 0;
        getmaxyx(stdscr, max_y, max_x);
        mvprintw(0, max_x - 10, "LEVEL: %zu", feed.snake.tsize);
    }

    void printExit() {
        int max_x = 0, max_y = 0;
        getmaxyx(stdscr, max_y, max_x);
        mvprintw(max_y / 2, max_x / 2 - 5, "Your LEVEL is %zu", feed.snake.tsize);
    }
...
public:
    ...
    void run() {
        while (key_pressed != STOP_GAME) {
            key_pressed = getch();
            changeDirection(feed.snake.direction, key_pressed);
            
            if (feed.snake.isCrash()) 
                break;
